...
/** Repository for managing {@link Bid} entities and custom bid queries. */
@Repository
public interface BidRepository extends JpaRepository<Bid, Long> {

  // JPA Derived Query Method. Syntax: [Prefix] + [Subject] + [By]
  ...
  // Property Expression: Must be a variable name in your class (the class here is Bid)
  ...
  Page<Bid> findItemBidHistory(Pageable pageable, @Param("itemId") Long itemId);

  @NativeQuery(
      value =
          "SELECT bids.* FROM bids"
              + " INNER JOIN items ON bids.item_id = items.item_id"
              + " INNER JOIN item_statuses ON items.item_id = item_statuses.item_id"
              + " WHERE :username = bids.bidder_username"
              + " AND item_statuses.end_time < :now")
