...
public interface BidRepository extends JpaRepository<Bid, Long> {
  ...
  @NativeQuery(
      value =
          "SELECT bids.* FROM bids INNER JOIN items ON bids.item_id = items.item_id INNER JOIN"
              + " item_statuses ON items.item_id = item_statuses.item_id WHERE :username ="
              + " bids.bidder_username AND item_statuses.end_time < :now")
