...
/**
 * Thành phần quản lý luồng dữ liệu phản kháng (Reactive Stream) cho số dư của người dùng.
 * Cho phép phát và lắng nghe các thay đổi số dư theo thời gian thực (real-time).
 */
@Component
public class UserBalanceSink {

    // Bản đồ lưu trữ các sink phát dữ liệu số dư cho từng người dùng, sử dụng ConcurrentHashMap để đảm bảo an toàn đa luồng (thread-safe)
    ...
    public Flux<Double> getBalanceSink(String username) {
        ...
        return sink.asFlux().doFinally(signalType -> {
            balanceSinksMap.computeIfPresent(username, (id, existingSink) -> {
                // Nếu không còn subscriber nào lắng nghe thì xóa sink này khỏi bản đồ để giải phóng bộ nhớ
